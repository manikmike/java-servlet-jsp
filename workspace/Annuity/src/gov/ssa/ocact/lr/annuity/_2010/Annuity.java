package gov.ssa.ocact.lr.annuity._2010;
...
public class Annuity {
   ...
   private static final int TRyear = 2010;
   ...
   static {
	   ...
	   String filename[] = {"qximorascext.hist", "qximorascext.alt1",
			   "qximorascext.alt2", "qximorascext.alt3"};
	   ...
	   for (int alt = 0; alt < nalt; alt++)
	   {
		   try {
			   ...
			   BufferedReader qxifile = new BufferedReader(new FileReader(path + filename[alt]));
   ...
   void printTableData (char tableType){
      switch (tableType){
         ...
         case 'J':
            ...
            if (OutputFormat == 1) {
            	if ((age-begin_age+1)%5 == 0 && (age-begin_age+1)%60 !=0 && (age+1) !=100) out.println();
            }
            if (OutputFormat == 1) {
            	if ((age-begin_age+1)%60 == 0 || (age-begin_age+1) == 100 || (age) == end_age) out.println(dashes);      
            }
      }
   }
