...
public class Annuity {
   ...
   private static final int nyea = 302;
   ...
   static {
	   ...
	   String filename[] = {"qximorj.hist", "qximorj.alt1",
			   "qximorj.alt2", "qximorj.alt3"};
	   ...
	   for (int alt = 0; alt < nalt; alt++)
	   {
		   try {
			   ...
			   RandomAccessFile qxifile = new RandomAccessFile(path + filename[alt], "r");
               FileChannel roChannel = qxifile.getChannel();
               ByteBuffer qxiBuffer = roChannel.map(FileChannel.MapMode.READ_ONLY, 0, (int)roChannel.size());
	           // qxiBuffer.order(ByteOrder.LITTLE_ENDIAN);			   
			   ...
			   for (int year = startYear[alt]; year <= endYear[alt]; year++)
			   {
				   for (int age = -1; age < nage; age++)
				   {
					   for (int sex = 0; sex < nsex; sex++)
					   {
						   if (age == -1)
						   {
							   sepFactor[sex][year][alt] = qxiBuffer.getDouble();
							   continue;
						   }
						   qximor[age][sex][year][alt] = qxiBuffer.getDouble();						   
					   }
				   }
			   }
			   qxifile.close();
   ...
   public void annuity () {
      out.println("<HTML><HEAD><TITLE>INTERACTIVE LIFE TABLE AND ANNUITY PROGRAM</TITLE></HEAD><BODY><PRE>");
   ...
   void constructLifeTable(){
      ...
      for (age=0; age<=nage-1; age++){
         ...
         if (age+1 <= nage-1) lx[age+1] = lx[age] - dx[age];
   ...
   void printLifeTable(){
      ...
      if ( (OutputFormat == 1) & (((age+1)%60 ==1) | (age == begin_age)) ){
   ...
   void jointAnnuity(){
         ...
         for (int idiff=1; idiff<=ndiff; idiff++){
            ...
            lx[19] = 100000;
            int temp = year;
            // This will fix the problem with period mortality joint annuities
            // if (mort.equals("COH")) year = year - diff;
            // Uncomment the previous line and delete the next line
            year = year - diff;
            ...
            for (age=nage-2; age>=20; age=age-1){
               Nx[age] = Nx[age+1] + Dx[age];
               if (Dx[age] > 0) {
                  jntax[age][idiff] = Nx[age+1]/Dx[age];
               }
      ...
      switch (tableType){
         case 'S':
            out.println("\f" + "United States life table functions and actuarial functions at "+
                               ...
                               "\nbased on the Alternative " + alttag + " mortality probabilities used in the " +
                                TRyear + " Trustees Report\n" + "(assuming constant mortality before 1900 and after 2200)");
   ...
   void printTableData (char tableType){
      switch (tableType){
         case 'S':
            ...
            String ages;
            ages = Spaces.substring(0,(3-String.valueOf(age).length())).concat(String.valueOf(age));
            String lxs;
            ...
            mafx = 12f*(ax[age]+13f/24f);
            DecimalFormat mafs = new DecimalFormat("##0.00");
            mafxs = Spaces.substring(0,(9-String.valueOf(mafs.format(mafx)).length())).concat(String.valueOf(mafs.format(mafx)));
         ...
         case 'J':
            ...
            String lineOfOutput = new String(String.valueOf(" " + age));
            String jntaxs;
            for (int idiff=1; idiff<=ndiff; idiff++){
            ...
            if ((age+1)%5 == 0 & (age+1)%60 !=20 & (age+1) !=100) out.println();
            if ((age+1)%60 == 20 | (age+1) == 100) out.println(dashes);      
      }
   }
